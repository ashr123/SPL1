...
void Environment::start()
{
	...
	while (true)
	{
		...
		if (firstS == "ls")
		{
			commandsHistory.push_back(new LsCommand(secS.append(thirdS.empty() ? "" : " " + thirdS)));
			commandsHistory[commandsHistory.size() - 1]->execute(fs);
			continue;
		}
		if (firstS == "mkdir")
		...
		if (firstS == "mkfile")
		{
			commandsHistory.push_back(new MkfileCommand(secS.append(thirdS.empty() ? "" : " " + thirdS)));
			commandsHistory[commandsHistory.size() - 1]->execute(fs);
			continue;
		}
		if (firstS == "cp")
		{
			commandsHistory.push_back(new CpCommand(secS.append(thirdS.empty() ? "" : " " + thirdS)));
			commandsHistory[commandsHistory.size() - 1]->execute(fs);
			continue;
		}
		if (firstS == "mv")
		{
			commandsHistory.push_back(new MvCommand(secS.append(thirdS.empty() ? "" : " " + thirdS)));
			commandsHistory[commandsHistory.size() - 1]->execute(fs);
			continue;
		}
		if (firstS == "rename")
		{
			commandsHistory.push_back(new RenameCommand(secS.append(thirdS.empty() ? "" : " " + thirdS)));
			commandsHistory[commandsHistory.size() - 1]->execute(fs);
			continue;
		}
		if (firstS == "rm")
...
void Environment::copy(const Environment &other)
{
	...
	{
		commandsHistory.push_back(dynamic_cast<HistoryCommand *>(command) ?
								  ((HistoryCommand *) command)->clone(commandsHistory) :
								  dynamic_cast<ExecCommand *>(command) ?
								  ((ExecCommand *) command)->clone(commandsHistory) :
								  command->clone());
	}
}
